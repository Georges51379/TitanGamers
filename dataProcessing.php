...
if (isset($_POST['signup'])){
  ...
  if(count($errors) === 0){
    ...
    $status = "notverified";
    $userStatus = 1;
    $insert_query = mysqli_query($con,"INSERT INTO users (name, email,phone, password, code, status, userStatus)
                    values('$name', '$email','$phone', '$hashedpwd', '$code', '$status', 1)");
    if($insert_query){
...
//LOGIN CHECK (INDEX.PHP)
  if(isset($_POST['login'])){
    $email = $_POST['email'];
    $password = $_POST['password'];
    $query = mysqli_query($con, "SELECT * FROM users WHERE email = '$email' AND userStatus = 1 ");
    $row = mysqli_num_rows($query);
    if($row > 0){
