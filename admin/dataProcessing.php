...
//CHECK CODE VERIFICATION SECTION (USER-OTP.PHP)
  if(isset($_POST['check-code'])){
    $_SESSION['info'] = "";
    $otp_code = $_POST['otp'];
    $query_check_code = mysqli_query($con, "SELECT * FROM admin WHERE code = '$otp_code'");
...
//LOGIN CHECK (INDEX.PHP)
  if(isset($_POST['login'])){
    ...
    $query = mysqli_query($con, "SELECT * FROM admin WHERE ad_email = '$email' AND adminStatus = 1 ");
...
if(isset($_POST['forgotpwd'])){
  $email = $_POST['email'];
  $query = mysqli_query($con, "SELECT * FROM admin WHERE ad_email = '$email'");
...
if(isset($_POST['change-password'])){
  ...
  if($password !== $cpassword){
    $errors['password'] = "Password do no match!!!";
      ...
      $code = 0;
      $email = $_SESSION['ad_email']; //getting this email using session
      $hashedpwd = password_hash($password, PASSWORD_BCRYPT);
      $update_query = mysqli_query($con, "UPDATE admin SET code = '$code', password = '$hashedpwd' WHERE ad_email = '$email'");
      if($update_query){
