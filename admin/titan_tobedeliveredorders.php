...
$query=mysqli_query($con,"SELECT users.email AS userEmail,users.phone AS phone,users.shippingAddress AS shippingaddress,users.shippingCity AS shippingcity,
	...
	orders.toBeDelivered,products.productPrice AS productprice,orders.id AS id  FROM orders JOIN users ON  orders.userEmail=users.email JOIN products ON products.productName=orders.productName
WHERE orders.toBeDelivered IS NOT NULL");
$cnt=1;
...
?>
										<tr id="custom_tr">
											...
											<td><?php echo htmlentities($row['toBeDelivered']);?></td>
