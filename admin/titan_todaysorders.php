...
<center>
	<div class="container">
							<div class="titlewrapper">
								<h3 class="title">today's Orders</h3>
							</div>
...
$query=mysqli_query($con,"SELECT users.email AS userEmail,users.phone AS phone,users.shippingAddress AS shippingaddress,users.shippingCity AS shippingcity,
	users.shippingState AS shippingstate,users.shippingPinCode AS shippingpincode,products.productName AS productname,products.shippingCharge AS shippingcharge,orders.quantity
	 AS quantity, orders.orderDate AS orderdate,products.productPrice AS productprice,orders.id AS id  FROM orders JOIN users ON  orders.userEmail=users.email
