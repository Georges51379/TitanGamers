...
if(isset($_POST['submit']))
{
  $componentName=$_POST['componentName'];
	...
	$componentToken=$_GET['componentToken'];

$sql=mysqli_query($con,"UPDATE components SET componentName='$componentName', componentStatus= '$componentStatus', componentUpdateDate='$currentTime'
									WHERE componentToken = '".$_GET['componentoken']."'");
$_SESSION['msg']="Component Updated !!";
header("location:titan_component.php");
}

?>
<html lang="en">
<head>
	<title>Admin | Edit Component</title>
...
<?php
$query=mysqli_query($con,"SELECT * FROM components WHERE componentToken = '".$_GET['componentoken']."'");
while($row=mysqli_fetch_array($query))
{
?>
                  <div class="adminform_div">
                    <label class="adminform_label" for="basicinput">name</label><br>
                    <input type="text"  name="componentName"  placeholder="Enter Name"  value="<?php echo htmlentities($row['componentName']); ?>" class="adminform_input" required>
                  </div>

                  <div class="adminform_div">
                    <label class="adminform_label" for="basicinput"> status</label><br>
                    <select name="componentStatus"  id="componentStatus" class="adminform_select" required>
                      <option value="<?php echo htmlentities($row['componentStatus']); ?>"><?php echo htmlentities($row['componentStatus']); ?></option>
